...
class BandwidthMonitor(app_manager.RyuApp):
    ...
    def statsReplyHandler(self, ev):
        ...
        # [ ADD YOUR CODE HERE ]
        #print(self.topo.ports)
        #print(f'self.topo.ports[name].keys() {self.topo.ports[name].keys()}')
        #for key in self.topo.ports[name].keys():
        #    ifprint(f'key: {key}')
        #if name in self.topo.ports[name].keys():
        #    print(f'name {name}')
        #print(f'type of body element: {type(body[0])}')
        #print(f'this switch: {name}')
        #print(f'{body}')
        #print(f'self.topo.ports[name].keys() {self.topo.ports[name].keys()}')
        for portStat in body:
            if portStat.port_no in self.topo.ports[name].keys() and self.topo.ports[name][portStat.port_no].startswith("h"):
                #print(f'Adding Port Stat... name: {self.topo.ports[name][portStat.port_no]} tx_bytes: {portStat.tx_bytes} rx_bytes: {portStat.rx_bytes}')
                self.bwstats.addHostBwStat(self.topo.ports[name][portStat.port_no], portStat.tx_bytes, portStat.rx_bytes)
