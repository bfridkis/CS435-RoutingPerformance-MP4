        ...
        for edge in topo.edgeSwitches.values():
            ...
            for h in topo.hosts.values():
                ...
                if h.name in edge.neighbors:
                    ...
                    #find host's vlan (or first one, if multiple)
                    #print(topo.vlans)
                    for vlan in topo.vlans.keys():
                        if h.name in topo.vlans[vlan]:
                            hostVlan = vlan
                            break
                    outport = topo.ports[edge.name][topo.getVlanCore(hostVlan)]
